...
//上传图片
    public function _upload() {
        ...
        if (!$img) {
            ...
            $temp_size = getimagesize($path .$img['Filedata']['savepath'].$img['Filedata']['savename']);
            if ($temp_size[0] < 100 || $temp_size[1] < 100) {//判断宽和高是否符合头像要求
                return array('status' => 0, 'info' => '图片宽或高不得小于100px！');
            }
            return array('status' => 1, 'path' => UPLOADS_PATH .$img['Filedata']['savepath'].$img['Filedata']['savename']);
        }
    }
